...
from mobilevit_v3_v1_MOE_Pixel2_Enhanced import MobileViTv3_v1_dynamicFPN_MOE_Pixel2_Enhanced
from mobilevit_v3_v1_MOEV2_Pixel2_Enhanced import MobileViTv3_v1_dynamicFPN_MOEV2_Pixel2_Enhanced
...
def main():
    try:
        ...
        model_type = "MbViTMOE3_XS4_FF8_MF_Copper_Normalisation_NW_W" #'UNETred_copper_50_50_BruitDuet_fixe_NW_et_W'
        ...
        try:
            ...
            Model = MobileViTv3_v1_dynamicFPN_MOE_Pixel2_Enhanced((4096, 64), 'x_small', 1000, (32,32),noise_std=0.0).to(device)
            #Model = MobileViTv3_v1_dynamicFPN((4096, 64), 'xx_small4', 1000, (32,32)).to(device)
            #Model = UNETred().to(device)
            #Model = torch.compile(Model)
            pretrained_path ="/mnt/STORAGE-1/home/rniddam/UNET_Binary/MobileUnet_V2_modif/Test_End_seed_lr_MbViTPixel2_MOE_3E_XXSMALL4_noise00_FF8_Normal_MF_batchsize16_NW_W_BruitFixe_BruitDuet200e-6_20251209_031212_epochs_intermediaire_500_epochs_seed_69738009/epoch_25/Model.pth"
                    ...
                    for i in range(reconV.size(0)):
                        ...
                        if aux_stats is not None:
                            ...
                            if batch_idx == 0 and i == 0:
                                ...
                                for key in aux_stats.keys():
                                    if isinstance(aux_stats[key], torch.Tensor):
                                        print(f"  Debug {key} shape: {aux_stats[key].shape}")
                            ...
                            if 'expert_indices' in aux_stats:
                                ...
                                if expert_indices_sample is not None:
                                    ...
                                    # Stocker les probabilites de gating par couche si disponibles
                                    gate_probs_per_layer = []
                                    topk_values_per_layer = []

                                    # Extraire gate_probs et topk_values si disponibles
                                    gate_probs_sample = None
                                    topk_values_sample = None
                                    if 'gate_probs' in aux_stats:
                                        gate_probs_batch = aux_stats['gate_probs'].cpu().numpy()
                                        if gate_probs_batch.shape[0] > i:
                                            gate_probs_sample = gate_probs_batch[i]  # [num_layers, S, num_experts]
                                    if 'topk_values' in aux_stats:
                                        topk_values_batch = aux_stats['topk_values'].cpu().numpy()
                                        if topk_values_batch.shape[0] > i:
                                            topk_values_sample = topk_values_batch[i]  # [num_layers, S, top_k]
                                    ...
                                    for layer_idx in range(num_layers):
                                        ...
                                        # store gating probabilities for this layer
                                        if gate_probs_sample is not None:
                                            gate_probs_per_layer.append(gate_probs_sample[layer_idx])  # [S, num_experts]
                                        if topk_values_sample is not None:
                                            topk_values_per_layer.append(topk_values_sample[layer_idx])  # [S, top_k]
                                    ...
                                    # Calculer des statistiques supplementaires
                                    # Entropie de la distribution des experts (mesure de diversite)
                                    expert_entropy = 0.0
                                    if total_tokens > 0:
                                        for expert_id in range(num_experts):
                                            p = expert_fraction_sample[expert_id]
                                            if p > 0:
                                                expert_entropy -= p * np.log(p + 1e-10)

                                    # Calculer la variance de l'utilisation (mesure d'equilibre)
                                    expert_variance = np.var(expert_fraction_sample) if num_experts > 1 else 0.0

                                    # store all fields for this sample
                                    ...
                                    sample_expert_data['expert_entropy'] = float(expert_entropy)  # Entropie de la distribution
                                    sample_expert_data['expert_variance'] = float(expert_variance)  # Variance de l'utilisation
                                    ...
                                    # Ajouter les probabilites de gating si disponibles
                                    if gate_probs_per_layer:
                                        sample_expert_data['gate_probs_per_layer'] = gate_probs_per_layer  # Liste de [S, num_experts]
                                    if topk_values_per_layer:
                                        sample_expert_data['topk_values_per_layer'] = topk_values_per_layer  # Liste de [S, top_k]
                                    ...
                                    sample_expert_data['expert_entropy'] = 0.0
                                    sample_expert_data['expert_variance'] = 0.0
                                    sample_expert_data['num_experts'] = int(num_experts)
                                    sample_expert_data['num_layers'] = 0
                                    sample_expert_data['total_tokens'] = 0
        ...
        if all_expert_data:
            ...
            expert_entropy_array = np.zeros(num_samples, dtype=np.float32)
            expert_variance_array = np.zeros(num_samples, dtype=np.float32)
            ...
            for idx, data in enumerate(all_expert_data):
                ...
                if 'expert_entropy' in data:
                    expert_entropy_array[idx] = data['expert_entropy']
                if 'expert_variance' in data:
                    expert_variance_array[idx] = data['expert_variance']
            ...
            expert_summary = {
                ...
                'expert_entropy': expert_entropy_array,  # [num_samples] - entropie de la distribution des experts
                'expert_variance': expert_variance_array,  # [num_samples] - variance de l'utilisation des experts
            ...
            # Ajouter les probabilites de gating et topk_values si disponibles
            gate_probs_all = []
            topk_values_all = []
            for idx, data in enumerate(all_expert_data):
                if 'gate_probs_per_layer' in data:
                    gate_probs_all.append(data['gate_probs_per_layer'])
                else:
                    gate_probs_all.append([])
                if 'topk_values_per_layer' in data:
                    topk_values_all.append(data['topk_values_per_layer'])
                else:
                    topk_values_all.append([])

            if gate_probs_all and len(gate_probs_all[0]) > 0:
                gate_probs_cell = np.empty((num_samples,), dtype=object)
                for idx, probs_list in enumerate(gate_probs_all):
                    if probs_list:
                        layer_cell = np.empty((len(probs_list),), dtype=object)
                        for layer_idx, layer_probs in enumerate(probs_list):
                            layer_cell[layer_idx] = layer_probs
                        gate_probs_cell[idx] = layer_cell
                    else:
                        gate_probs_cell[idx] = np.array([], dtype=object)
                expert_summary['gate_probs_per_layer'] = gate_probs_cell

            if topk_values_all and len(topk_values_all[0]) > 0:
                topk_values_cell = np.empty((num_samples,), dtype=object)
                for idx, values_list in enumerate(topk_values_all):
                    if values_list:
                        layer_cell = np.empty((len(values_list),), dtype=object)
                        for layer_idx, layer_values in enumerate(values_list):
                            layer_cell[layer_idx] = layer_values
                        topk_values_cell[idx] = layer_cell
                    else:
                        topk_values_cell[idx] = np.array([], dtype=object)
                expert_summary['topk_values_per_layer'] = topk_values_cell
