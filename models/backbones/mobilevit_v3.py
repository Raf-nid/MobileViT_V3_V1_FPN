...
class MobileViTBlockV3_v1(nn.Module):
    def __init__(self, inp, attn_dim, ffn_multiplier, heads, dim_head, attn_blocks, patch_size):
        ...
        for i in range(attn_blocks):
            ffn_dim = ffn_dims[i]
            self.global_rep.add_module(f'TransformerEncoder_{i}', TransformerEncoder(attn_dim, ffn_dim, heads)) #, dim_head, dropout=0.3, attn_dropout=0.3)) #ajout du dropout ici de 0.1
...
class MobileViTv3_v1(nn.Module):
    def __init__(self, image_size, mode, num_classes, patch_size=(32, 32)):  
        ...
        assert mode in ['xx_small', 'perso', 'x_small', 'small']
...
class MobileViTv3_v1_dynamicFPN(nn.Module):
    def __init__(self, image_size, mode, num_classes, patch_size=(32, 32)):  
        """
        Implementation of MobileViTv3 based on v1
        """
        ...
        if mode == 'xx_small':
            ...
            mv2_exp_mult = 3 #4 ici normalement
            ffn_multiplier = 2
            last_layer_exp_factor = 4
            channels = [16, 24, 36, 80, 120, 144] #moyenne entre x_small et xx_small
            attn_dim = [80, 100, 120]
        elif mode == 'x_small':
            mv2_exp_mult = 2 #4 ici normalement
            ffn_multiplier = 2
            last_layer_exp_factor = 4
            channels = [16, 32, 48, 96, 160, 160]
            attn_dim = [96, 120, 144]
...
#model = MobileViTv3_v1_dynamicFPN((4096, 1024), 'x_small', 1000,patch_size=(128, 128))
