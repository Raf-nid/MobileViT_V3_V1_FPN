...
class MobileViTv3_v1_dynamicFPN6(nn.Module):
    def __init__(self, image_size, mode, num_classes, patch_size=(64, 64)):  
        ...
        self.layer_3 = nn.Sequential(
            InvertedResidual(channels[2], channels[3], stride=2, expand_ratio=mv2_exp_mult),
            MobileViTBlockV3_v1(channels[3], channels[3], ffn_multiplier, heads=4, dim_head=8, attn_blocks=2, patch_size=patch_size)
        )
        self.layer_4 = nn.Sequential(
            InvertedResidual(channels[3], channels[4], stride=2, expand_ratio=mv2_exp_mult),
            MobileViTBlockV3_v1(channels[4], channels[4], ffn_multiplier, heads=4, dim_head=8, attn_blocks=4, patch_size=patch_size)
        )
        self.layer_5 = nn.Sequential(
            InvertedResidual(channels[4], channels[5], stride=2, expand_ratio=mv2_exp_mult),
            MobileViTBlockV3_v1(channels[5], channels[5], ffn_multiplier, heads=4, dim_head=8, attn_blocks=3, patch_size=patch_size)
        )

        self.top_layer = nn.Conv2d(channels[5], 256, kernel_size=1)

        self.lateral_layers = nn.ModuleList([
            nn.Conv2d(1, 256, kernel_size=1),  # pour lat_0 ! 
            nn.Conv2d(channels[2], 256, kernel_size=1),
            nn.Conv2d(channels[3], 256, kernel_size=1),
            nn.Conv2d(channels[4], 256, kernel_size=1)
        ])

        # 5 smooth layers : p5->p1 + le dernier p0
        self.smooth_layers = nn.ModuleList([
            nn.Conv2d(256, 256, kernel_size=3, padding=1) for _ in range(6)
        ])
    ...
    def forward(self, x):
        lat_0 = x
        x = self.conv_0(x)          # stride 2

        lat_2 = self.layer_2(x)     # stride 4
        lat_3 = self.layer_3(lat_2) # stride 8
        lat_4 = self.layer_4(lat_3) # stride 16
        lat_5 = self.layer_5(lat_4) # stride 32

        # FPN top-down
        p5 = self.top_layer(lat_5)
        p4 = self._upsample_add(p5, self.lateral_layers[3](lat_4))
        p3 = self._upsample_add(p4, self.lateral_layers[2](lat_3))
        p2 = self._upsample_add(p3, self.lateral_layers[1](lat_2))

        # Nouveau p0 avec lateral_layer(lat_0)
        p0 = self._upsample_add(p2, self.lateral_layers[0](lat_0))

        # Smooth layers
        p5 = self.smooth_layers[0](p5)
        p4 = self.smooth_layers[1](p4)
        p3 = self.smooth_layers[2](p3)
        p2 = self.smooth_layers[3](p2)
        p0 = self.smooth_layers[5](p0)  # Smooth p0
...
model = MobileViTv3_v1_dynamicFPN6((4096, 1024), 'xx_small', 1000, (128, 128))
