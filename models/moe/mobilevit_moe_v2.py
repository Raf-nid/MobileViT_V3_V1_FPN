...
class MoEFeedForward(nn.Module):
    ...
    """
    def __init__(self, dim, hidden_dim, num_experts=3, top_k=1, dropout=0.0, noise_std=0.0):
        super().__init__()
...
class TransformerEncoder(nn.Module):
    """Transformer Encoder avec option MoE"""
    def __init__(self, embed_dim, ffn_latent_dim, heads=8, dim_head=8, 
        ...
        self.pre_norm_mha = nn.LayerNorm(embed_dim, eps=1e-5)
        self.attn = Attention(embed_dim, heads, dim_head, attn_dropout)
        self.attn_dropout = nn.Dropout(dropout)
        
        # Feed-forward network with pre-norm
        self.pre_norm_ffn = nn.LayerNorm(embed_dim, eps=1e-5)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[Dict[str, torch.Tensor]]]:
        ...
        x = x + self.attn_dropout(self.attn(self.pre_norm_mha(x)))
        ...
        if not self.use_moe:
            ...
            ffn_out, aux = self.ffn(self.pre_norm_ffn(x))
            x = x + ffn_out
            return x, aux
...
class MobileViTBlockV3_v1(nn.Module):
    ...
    def __init__(self, inp, attn_dim, ffn_multiplier, heads, dim_head, 
        ...
        for i in range(attn_blocks):
            # MoE only in the last transformer of this block
            is_last_block = (i == attn_blocks - 1) and use_moe
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[Dict[str, torch.Tensor]]]:
        ...
        # Aggregate aux stats (prendre le dernier uniquement)
...
class MobileViTv3_v1_dynamicFPN_MOE(nn.Module):
    """
...
class MoELoss(nn.Module):
    ...
    """
    def __init__(self, lambda_balance: float = 0.01, lambda_entropy: float = 0.0):
        super().__init__()
...
if __name__ == "__main__":
    input_tensor = torch.randn(1, 1, 4096, 1024)  # reduce size for test
    model = MobileViTv3_v1_dynamicFPN_MOE((4096, 1024), 'xx_small4', num_classes=1000, patch_size=(32, 32))
