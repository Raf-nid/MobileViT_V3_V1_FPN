...
class MoEFeedForward(nn.Module):
    ...
    """
    def __init__(self, dim, hidden_dim, num_experts=2, top_k=1, dropout=0.0, noise_std=0.1):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        aux['expert_indices'] = topk_idx  # [B, S, top_k] - indices des experts selectionnes
        aux['gate_probs'] = gate_probs  # [B, S, num_experts] - probabilites de gating pour tous les experts
        aux['topk_values'] = topk_vals_normalized  # [B, S, top_k] - normalized weights for selected experts
        aux['gate_logits'] = gate_logits  # [B, S, num_experts] - logits bruts du gating
...
class TransformerEncoder(nn.Module):
    def __init__(self, embed_dim, ffn_latent_dim, heads=8, dim_head=8, 
                 ...
                 num_experts=2, top_k=1, noise_std=0.0):
        super().__init__()
...
class MobileViTBlockV3_v1(nn.Module):
    ...
    def __init__(self, inp, attn_dim, ffn_multiplier, heads, dim_head, 
                 attn_blocks, patch_size, use_moe=False, num_experts=2, 
                 top_k=1, noise_std=0.0, num_moe_layers=2):
        ...
        for i in range(attn_blocks):
            # MoE only in the first num_moe_layers encoder blocks
            use_moe_for_layer = use_moe and (i < num_moe_layers)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[Dict[str, torch.Tensor]]]:
        ...
        if aux_list:
            B_orig = info_dict['batch_size']
            patch_area = self.patch_area
            
            # Les expert_indices ont la forme [B*patch_area, S, top_k] pour chaque layer
            # On doit les reorganiser en [B, num_encoder_layers, S, top_k]
            expert_indices_list = []
            gate_probs_list = []
            topk_values_list = []
            
            for idx, a in enumerate(aux_list):
                try:
                    # Traiter expert_indices
                    if 'expert_indices' in a and a['expert_indices'] is not None:
                        expert_idx = a['expert_indices']  # [B*patch_area, S, top_k]
                        B_expanded, S, top_k = expert_idx.shape
                        
                        if B_expanded == B_orig * patch_area:
                            expert_idx_reshaped = expert_idx.view(B_orig, patch_area, S, top_k)
                            # Prendre le premier patch de chaque echantillon pour simplifier
                            expert_idx_batch = expert_idx_reshaped[:, 0, :, :]  # [B, S, top_k]
                        else:
                            expert_idx_batch = expert_idx[:B_orig, :, :]  # [B, S, top_k]
                        expert_indices_list.append(expert_idx_batch)
                    
                    # Traiter gate_probs (probabilites pour tous les experts)
                    if 'gate_probs' in a and a['gate_probs'] is not None:
                        gate_probs = a['gate_probs']  # [B*patch_area, S, num_experts]
                        B_expanded, S, num_experts = gate_probs.shape
                        
                        if B_expanded == B_orig * patch_area:
                            gate_probs_reshaped = gate_probs.view(B_orig, patch_area, S, num_experts)
                            gate_probs_batch = gate_probs_reshaped[:, 0, :, :]  # [B, S, num_experts]
                        else:
                            gate_probs_batch = gate_probs[:B_orig, :, :]  # [B, S, num_experts]
                        gate_probs_list.append(gate_probs_batch)
                    
                    # handle topk_values (normalized expert weights)
                    if 'topk_values' in a and a['topk_values'] is not None:
                        topk_vals = a['topk_values']  # [B*patch_area, S, top_k]
                        B_expanded, S, top_k = topk_vals.shape
                        
                        if B_expanded == B_orig * patch_area:
                            topk_vals_reshaped = topk_vals.view(B_orig, patch_area, S, top_k)
                            topk_vals_batch = topk_vals_reshaped[:, 0, :, :]  # [B, S, top_k]
                        else:
                            topk_vals_batch = topk_vals[:B_orig, :, :]  # [B, S, top_k]
                        topk_values_list.append(topk_vals_batch)
                        
                except Exception as e:
                    import warnings
                    warnings.warn(f"Erreur lors du traitement des donnees MoE pour layer {idx}: {e}")
            ...
            aux_stats = {
                ...
                'info_dict': info_dict  # Ajouter info_dict pour reference
            }
            
            # Ajouter les donnees detaillees si disponibles
            if expert_indices_list:
                aux_stats['expert_indices'] = torch.stack(expert_indices_list, dim=1)  # [B, num_encoder_layers, S, top_k]
            if gate_probs_list:
                aux_stats['gate_probs'] = torch.stack(gate_probs_list, dim=1)  # [B, num_encoder_layers, S, num_experts]
            if topk_values_list:
                aux_stats['topk_values'] = torch.stack(topk_values_list, dim=1)  # [B, num_encoder_layers, S, top_k]
        else:
            aux_stats = None
...
class MobileViTv3_v1_dynamicFPN_MOEV2_Pixel2_Enhanced(nn.Module):
    """
    MobileViT v3 avec FPN dynamique et Mixture of Experts (Version Enhanced)
    MoE placed only in layer_5 for efficiency
    Enhanced version with full expert statistics collection
    """
    def __init__(self, image_size: Tuple[int, int], mode: str, 
                 num_classes: int = 1000, patch_size: Tuple[int, int] = (64, 64),
                 num_experts: int = 2, top_k: int = 1, noise_std: float = 0.0):
        super().__init__()
        ...
        # Layer 5 MoE (first two TransformerEncoder layers only)
        self.layer_5 = nn.Sequential(
            ...
            MobileViTBlockV3_v1(
                ...
                noise_std=noise_std, num_moe_layers=2
            )
        )
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[Dict[str, torch.Tensor]]]:
        ...
        lat_3, _ = self.layer_3(lat_2)
        lat_4, _ = self.layer_4(lat_3)
        lat_5, aux_stats = self.layer_5(lat_4)  # MoE dans layer_5
...
class MoELoss(nn.Module):
    ...
    """
    def __init__(self, lambda_balance: float = 0.0001, lambda_entropy: float = 0.00001):
        super().__init__()
...
if __name__ == "__main__":
    ...
    # model
    model = MobileViTv3_v1_dynamicFPN_MOEV2_Pixel2_Enhanced(
