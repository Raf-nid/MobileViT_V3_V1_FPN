...
class MobileNetV2_dynamicFPNPixel2(nn.Module):
    def __init__(self, width_mult=1.):
        super(MobileNetV2_dynamicFPNPixel2, self).__init__()
        ...
        # Final head: pixel shuffle + 1x1 to single channel
        ...
        self.last_conv = nn.Conv2d(2, 1, kernel_size=1, stride=1, padding=0)
    ...
    def _make_inverted_residual_block(self, expansion_factor, width_factor, n, stride):
        ...
        for i in range(n):
            # only the first inverted block uses the stage stride
    ...
    def forward(self, x):
        ...
        for i, block in enumerate(self.inverted_residual_blocks):
            output = block(x)
            # lateral connection only when spatial size shrinks
        ...
        # Use highest-resolution level (last in p_layers)
        ...
        out = self.pixel_shuffle_conv(highest_feature)  # 256 -> 16 channels
        out = self.pixel_shuffle(out)  # H/4 x W/4 -> H x W (1 ch)
...
if __name__ == '__main__':
    # Smoke test / optional TensorBoard graph
    net = MobileNetV2_dynamicFPNPixel2()
    #print(net)
    input_tensor = torch.randn(1, 1, 4096, 64)
    output = net(input_tensor)
    print("Output shape:", output.shape)  # expect [1, 1, 4069, 1024] for this dummy input
    # Optional TensorBoard graph:
    ...
    print(flops.total())
    param_dict = parameter_count(net)
    total_params = sum(param_dict.values())
    print(f"[fvcore] Total parameters: {total_params:,}")
