...
class DomainClassifier(nn.Module):
    def __init__(self, in_features, hidden_size=256):
        ...
        self.domain_layer = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_size, 2)  # 2 classes : source ou cible
        )
...
class MobileViTv3_v1_dynamicFPN_DANN(nn.Module):
    def __init__(self, image_size, mode, num_classes, patch_size=(64, 64)):  
        ...
        self.grl = GradientReversalLayer(lambda_=1.0)  # lambda ajustable
        self.domain_classifier = DomainClassifier(in_features=256)
    ...
    def forward(self, x, return_domain=False):
        ...
        x = self.top_layer(x)   # [B, 256, 1, 1]
        feat = torch.flatten(x, 1)  # [B, 256]

        domain_pred = self.domain_classifier(self.grl(feat))
        ...
        return out, domain_pred  # optionnel : retourne feat si tu veux l'utiliser ailleurs




input_tensor = torch.randn(1, 1, 4096, 1024)
model = MobileViTv3_v1_dynamicFPN_DANN((4096, 1024), 'xx_small', 1000, (32, 32))
# output, domain_pred = model(input_tensor)
# print(output.size())
