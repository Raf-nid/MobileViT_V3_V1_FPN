...
class MobileViTv3_v1_dynamicFPN2(nn.Module):
    def __init__(self, image_size, mode, num_classes, patch_size=(64, 64)):  
        ...
        # Upsample layers
        self.final_conv1 = nn.ConvTranspose2d(256, 256, kernel_size=4, stride=2, padding=1)
        self.final_conv2 = nn.Conv2d(256, 32, kernel_size=1, stride=1, padding=0)
        self.final_conv3 = nn.ConvTranspose2d(16, 16, kernel_size=4, stride=2, padding=1)
        self.final_conv4 = nn.Conv2d(8, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        out = self.final_conv1(smooth_features[-1])  # residual path
        out = self.final_conv2(out)  # add previous stage
        out  = DoubleConv(32, 16)(out)  # double conv on first stage
        out = self.final_conv3(out)  # another residual link
        out = DoubleConv(16, 8)(out)
        out = self.final_conv4(out)  # Finalisation de la sortie
        x= 2*x-1
        return out
...
#model = MobileViTv3_v1_dynamicFPN2((4096, 1024), 'xx_small', 1000, (32, 32))
