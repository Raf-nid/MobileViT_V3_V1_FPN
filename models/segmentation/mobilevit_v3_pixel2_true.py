...
class MobileViTv3_v1_dynamicFPNpixel2_V2(nn.Module):
    def __init__(self, image_size, mode, num_classes, patch_size=(64, 64)):  
        ...
        self.conv1x1 = nn.Conv2d(channels[-1], CC, kernel_size=1, stride=1)
        self.global_pool = nn.AdaptiveAvgPool2d(1)
        self.top_layer = nn.Conv2d(CC, 256, kernel_size=1, stride=1, padding=0)
        ...
        # pixel shuffle to restore H x W
        # smooth_features[-1] a une taille de H/4 x W/4 avec 256 channels
        # H/4 x W/4 -> H x W requires upsample factor 4
        ...
        self.last_conv = nn.Conv2d(2, 1, kernel_size=1, stride=1, padding=0)  # optional 1x1 conv to single channel
    ...
    def forward(self, x):
        ...
        # pixel shuffle to restore spatial size
...
if __name__ == "__main__":
    ...
    model = MobileViTv3_v1_dynamicFPNpixel2((4096, 64), 'xx_small4', 1000, (32, 32))
    ...
    # FLOPs (may be incomplete if some ops are unsupported)
    try:
        flops = FlopCountAnalysis(model, input_tensor)
