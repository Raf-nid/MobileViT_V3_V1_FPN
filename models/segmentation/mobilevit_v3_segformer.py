...
class MobileViTv3_v1_SegFormer(nn.Module):
    def __init__(self, image_size, mode, num_classes, patch_size=(64, 64)):  
        ...
        # Decoder layers with x0 included properly
        k = 32
        self.linear_c0 = nn.Conv2d(1, k, kernel_size=1)  # add this
        self.linear_c2 = nn.Conv2d(channels[2], k, kernel_size=1)
        self.linear_c3 = nn.Conv2d(channels[3], k, kernel_size=1)
        self.linear_c4 = nn.Conv2d(channels[4], k, kernel_size=1)
        self.linear_c5 = nn.Conv2d(channels[5], k, kernel_size=1)
        self.linear_fuse = nn.Conv2d(5*k, k, kernel_size=1)  # 5 × 256
            
        #self.linear_fuse = nn.Conv2d(1024, 256, kernel_size=1)
        self.dropout = nn.Dropout2d(0.1)
        self.linear_pred = nn.Conv2d(k, num_classes, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x = self.conv_0(x)
        x1 = self.layer_1(x)
        x2 = self.layer_2(x1)   # H/4
        x3 = self.layer_3(x2)   # H/8
        x4 = self.layer_4(x3)   # H/16
        x5 = self.layer_5(x4)   # H/32

        #print(f"x0: {x0.shape}, x1: {x1.shape}, x2: {x2.shape}, x3: {x3.shape}, x4: {x4.shape}, x5: {x5.shape}")
        _c0 = F.interpolate(self.linear_c0(x0), size=(H, W), mode='bilinear', align_corners=False)
        _c2 = F.interpolate(self.linear_c2(x2), size=(H, W), mode='bilinear', align_corners=False)
        _c3 = F.interpolate(self.linear_c3(x3), size=(H, W), mode='bilinear', align_corners=False)
        _c4 = F.interpolate(self.linear_c4(x4), size=(H, W), mode='bilinear', align_corners=False)
        _c5 = F.interpolate(self.linear_c5(x5), size=(H, W), mode='bilinear', align_corners=False)
        #print(f"_c0: {_c0.shape}, _c2: {_c2.shape}, _c3: {_c3.shape}, _c4: {_c4.shape}, _c5: {_c5.shape}")
        features = torch.cat([_c0, _c2, _c3, _c4, _c5], dim=1)  # shape: (B, 1280, H, W)
        fused = self.linear_fuse(features)
        fused = self.dropout(fused)
        out = self.linear_pred(fused)  # shape: (B, num_classes, H, W)
        return out
...
if __name__ == "__main__":
    ...
    model = MobileViTv3_v1_SegFormer((4096, 1024), 'xx_small', 1, (32, 32))
