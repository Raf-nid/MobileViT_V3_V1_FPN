...
from SPT2 import ShiftedPatchTokenization
...
class MobileViTBlockV3_v1(nn.Module):
    def __init__(self, inp, attn_dim, ffn_multiplier, heads, dim_head, attn_blocks, patch_size,shift_mode="3"):
        ...
        # ShiftedPatchTokenization remplace le module unfolding/folding standard
        self.use_spt = True
        self.spt = ShiftedPatchTokenization(
            in_dim=attn_dim,
            dim=attn_dim,
            merging_size=patch_size[0],
    ...
    def forward(self, x):
        ...
        if self.use_spt:
            ...
            _, info_dict = self.unfolding(fm_conv)
           
            # Passer à travers le transformer
            x = self.global_rep(x)
           
            # Redimensionner pour le folding
            #x = x.reshape(batch_size * self.patch_area, num_patches, -1)
            
            # Folding pour revenir à la forme spatiale
            x = self.folding(x, info_dict)
        else:
            # Méthode originale d'unfolding/folding
...
class MobileViTv3_SPT_v1_dynamicFPN(nn.Module):
    def __init__(self, image_size, mode, num_classes, patch_size=(64, 64),shift_mode="3"):  
        """
        Implementation of MobileViTv3 based on v1
        """
        super().__init__()
        ...
        self.layer_3 = nn.Sequential(
            InvertedResidual(channels[2], channels[3], stride=2, expand_ratio=mv2_exp_mult),
            MobileViTBlockV3_v1(channels[3], attn_dim[0], ffn_multiplier, heads=4, dim_head=8, attn_blocks=2, patch_size=patch_size,shift_mode=shift_mode)
        )
        self.layer_4 = nn.Sequential(
            InvertedResidual(channels[3], channels[4], stride=2, expand_ratio=mv2_exp_mult),
            MobileViTBlockV3_v1(channels[4], attn_dim[1], ffn_multiplier, heads=4, dim_head=8, attn_blocks=4, patch_size=patch_size,shift_mode=shift_mode)
        )
        self.layer_5 = nn.Sequential(
            InvertedResidual(channels[4], channels[5], stride=2, expand_ratio=mv2_exp_mult),
            MobileViTBlockV3_v1(channels[5], attn_dim[2], ffn_multiplier, heads=4, dim_head=8, attn_blocks=3, patch_size=patch_size,shift_mode=shift_mode)
        )
        #self.conv_1x1_exp = conv_2d(channels[-1], channels[-1]*last_layer_exp_factor, kernel_size=1, stride=1)
        #self.out = nn.Linear(channels[-1]*last_layer_exp_factor, num_classes, bias=True)
...
#model = MobileViTv3_SPT_v1_dynamicFPN((4096, 1024), 'xx_small', 1000, (32, 32),shift_mode='3')
