...
class UNetUpBlock(nn.Module):
    ...
    def forward(self, x, skip):
        ...
        # strict size match: crop skip if needed
...
class MobileViTv3_v1_UNET(nn.Module):
    def __init__(self, image_size, mode, num_classes, patch_size=(64, 64)):
        ...
        # U-Net style decoder
        self.up4 = UNetUpBlock(channels[5], channels[4], 128)
        self.up3 = UNetUpBlock(128, channels[3], 64)
        self.up2 = UNetUpBlock(64, channels[2], 32)
        self.up1 = UNetUpBlock(32, channels[1], 16)
        self.up0 = UNetUpBlock(16, channels[0], 16)

        # final upsampling to H with skip from raw input
        self.up_input = UNetUpBlock(16, 1, 16)
    ...
    def forward(self, x):
        ...
        x0 = self.conv_0(x)       # H/2
        x1 = self.layer_1(x0)     # H/2
        ...
        d2 = self.up2(d3, x2)
        d1 = self.up1(d2, x1)
        #d0 = self.up0(d1, x0)     # H/2

        d_out = self.up_input(d1, x)  # fuse with raw input at full resolution
        out = self.seg_head(d_out)   # Output final H × W
...
if __name__ == "__main__":
    ...
    model = MobileViTv3_v1_UNET((4096, 1024), 'xx_small', 1, (128, 128))
