...
class Conv2dChannelsLast(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, groups=1, bias=True, patch_size=(32, 32)):
        super().__init__()
    ...
    def forward(self, x):
        # x est de forme [B*P, N, C]
        batch_patch_size, num_patches, channels = x.shape
        batch_size = batch_patch_size // self.patch_area
        
        # Restructurer pour obtenir [B, C, H, W]
        # D'abord [B*P, N, C] -> [B, P, N, C]
        x = x.view(batch_size, self.patch_area, num_patches, channels)
        # [B, P, N, C] -> [B, C, N, P]
        x = x.permute(0, 3, 2, 1).contiguous()
        
        # Reconstruire les dimensions spatiales
        num_patch_h = int(math.sqrt(num_patches))  # Supposons que les patches sont carrés
        num_patch_w = num_patch_h
        
        # [B, C, N, P] -> [B, C, H, W]
        x = x.view(batch_size, channels, num_patch_h * self.patch_h, num_patch_w * self.patch_w)
        ...
        # Restructurer pour revenir à [B*P, N, C]
        out_channels = x.size(1)
        # [B, C', H, W] -> [B, C', N, P]
        x = x.view(batch_size, out_channels, num_patches, self.patch_area)
        # [B, C', N, P] -> [B, P, N, C']
        x = x.permute(0, 3, 2, 1).contiguous()
        # [B, P, N, C'] -> [B*P, N, C']
        x = x.view(batch_size * self.patch_area, num_patches, out_channels)
...
class LongShortRangeAttention(nn.Module):
    def __init__(self, embed_dim, heads=4, dim_head=8, attn_dropout=0, kernel_size=3):
        ...
        self.long_attn = Attention(self.half_dim, heads, dim_head, attn_dropout)
    ...
    def forward(self, x):
        ...
        x1 = x[:, :, :self.half_dim]  # Branche attention
        x2 = x[:, :, self.half_dim:]  # Branche convolution
        ...
        fusion = torch.cat([attn_out, conv_out], dim=-1)  # [B, S, embed_dim]
        return fusion
...
class MobileViTv3Litev3_v1_dynamicFPN(nn.Module):
...
#model = MobileViTv3Litev3_v1_dynamicFPN((4096, 1024), 'xx_small', 1000, (32, 32))
