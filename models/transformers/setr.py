...
class SegmentationTransformer(nn.Module):
    def __init__(
        ...
        self.num_patches = int((img_dim[0]//patch_dim) * (img_dim[1]//patch_dim))
        #print(f"Number of patches: {self.num_patches}")
        self.seq_length = self.num_patches
        self.flatten_dim = patch_dim * patch_dim * num_channels

        self.linear_encoding = nn.Linear(self.flatten_dim, embedding_dim)
        #print(f"Flatten dim: {self.flatten_dim}", f"Embedding dim: {embedding_dim}", f"Seq length: {self.seq_length}") 
    ...
    def _reshape_output(self, x):
        ...
        x = x.view(x.size(0), int(self.img_dim[0] //self.patch_dim), int(self.img_dim[1] // self.patch_dim), self.embedding_dim)
...
def SETR_Naive_S(dataset='fmc', _conv_repr=False, _pe_type="fixed"):
    if dataset.lower() == 'fmc':
        img_dim = [4096, 1024]
        num_classes = 1
...
def SETR_Naive_L(dataset='fmc', _conv_repr=False, _pe_type="fixed"):
    if dataset.lower() == 'fmc':
        img_dim = [4096, 1024]
        num_classes = 1
    elif dataset.lower() == 'ade20k':
...
def SETR_Naive_H(dataset='fmc', _conv_repr=False, _pe_type="fixed"):
    if dataset.lower() == 'fmc':
        img_dim = [4096, 1024]
        num_classes = 1
    elif dataset.lower() == 'ade20k':
...
def SETR_PUP_S(dataset='fmc', _conv_repr=False, _pe_type="fixed"):
    if dataset.lower() == 'fmc':
        img_dim = [4096, 1024]
        num_classes = 1
    elif dataset.lower() == 'ade20k':
...
def SETR_PUP_L(dataset='fmc', _conv_repr=False, _pe_type="fixed"):
    if dataset.lower() == 'fmc':
        img_dim = [4096, 1024]
        num_classes = 1
    elif dataset.lower() == 'ade20k':
...
def SETR_PUP_H(dataset='fmc', _conv_repr=False, _pe_type="fixed"):
    if dataset.lower() == 'fmc':
        img_dim = [4096, 1024]
        num_classes = 1
    elif dataset.lower() == 'ade20k':
...
def SETR_MLA_S(dataset='fmc', _conv_repr=False, _pe_type="fixed"):
    if dataset.lower() == 'fmc':
        img_dim = [4096, 1024]
        num_classes = 1
    elif dataset.lower() == 'ade20k':
...
def SETR_MLA_L(dataset='fmc', _conv_repr=False, _pe_type="fixed"):
    if dataset.lower() == 'fmc':
        img_dim = [4096, 1024]
        num_classes = 1
    elif dataset.lower() == 'ade20k':
...
def SETR_MLA_H(dataset='fmc', _conv_repr=False, _pe_type="fixed"):
    if dataset.lower() == 'fmc':
        img_dim = [4096, 1024]
        num_classes = 1
    elif dataset.lower() == 'ade20k':
...
# Example: initialize FMC-sized models (commented demo)
...
# Random input tensor (batch_size=1, channels=3, height=768, width=768)
# input_tensor = torch.randn(1, 1, 4096, 1024)

# # Test each model
# #for model_fn in models:
# model_fn = SETR_Naive_S
# aux_layers, model = model_fn(dataset='fmc')
# output = model(input_tensor)
# #output = model(input_tensor, auxillary_output_layers=aux_layers)
# print(f"{model_fn.__name__} output shape:", output.shape)
...
#for model_fn in models:
#    print(data)
#    aux_layers, model = model_fn(dataset=data)
#    
#    if aux_layers is not None:
#        print('ok')
#        output = model(input_tensor, auxillary_output_layers=aux_layers)
#        writer = SummaryWriter()
#        writer.add_graph(model, input_tensor,uxillary_output_layers=aux_layers)
#        writer.close()
#    else:
#        print('ok2')
#        output = model(input_tensor)
